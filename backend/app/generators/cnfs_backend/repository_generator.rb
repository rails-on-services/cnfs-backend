...
module CnfsBackend
  class RepositoryGenerator < Thor::Group
    ...
    include CnfsBackend::GeneratorConcern
    argument :context
    argument :repository

    def root_files
      y = YAML.load_file(views_path.join('repository.yml'))
      repository.update(y.deep_merge(config: { url: 'h' }))
      # binding.pry
      in_root { directory('files', '.') }
    end
    ...
    def services_gemfiles
      ...
      in_root do
        gemfile_extensions.each do |extension|
          template_file = "templates/#{path}#{extension}.erb"
          @extension = extension
    ...
    def dockerfiles
      ...
      in_root do
        dockerfile_extensions.each do |extension|
          @extension = extension
          template("templates/Dockerfile.erb", "Dockerfile#{extension}")
        end
      end
    end
